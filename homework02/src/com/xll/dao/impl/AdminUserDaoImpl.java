 ...
 * @version 1.0
 */
public class AdminUserDaoImpl extends BaseDao implements UserDao {
    ...
    public User find(String name, String password) {
        User user = null;
        String sql = "SELECT * FROM user_admin_table WHERE name=? AND password=?";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                System.out.println("DAO验证管理员: " + user);
            }
    ...
    public List<User> query(User condition) {
        ...
        String sql = "SELECT * FROM user_admin_table";
        if (condition != null) {
            sql += " WHERE 1=1";
    ...
    public int insert(User user) {
        int rows = 0;
        String sql = "INSERT INTO user_admin_table(name,password,createTime,status) VALUES(?,?,?,?) ";
    ...
    public int update(User user) {
        int rows = 0;
        String sql = "UPDATE user_admin_table SET name=?,status=? WHERE id=?";

        try {
            pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, user.getName());
            pstmt.setString(2, user.getStatus().getName());
    ...
    public int updateAccessTime(User user) {
        int rows = 0;
        String sql = "UPDATE user_admin_table SET lastAccessTime=? WHERE id=?";
    ...
    public int updatePwd(int id, String newPwd) {
        int rows = 0;
        String sql = "UPDATE user_admin_table SET password=? WHERE id=?";
    ...
    public int updateStatus(int id, String status) {
        int rows = 0;
        String sql = "UPDATE user_admin_table SET status=? WHERE id=?";
    ...
    public int delete(int id) {
        int rows = 0;
        String sql = "DELETE FROM user_admin_table WHERE id=?";
    ...
    public int count(User condition) {
        int num = 0;
        String sql = "SELECT count(*) FROM user_admin_table WHERE 1=1";
